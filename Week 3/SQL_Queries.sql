...
#2 In the client table, get an id value of the last client where the district_id is equal to 72.
SELECT client_id FROM client
...
#17 In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your outp
...
SELECT account_id, type, round(sum(amount),2) AS total_amount FROM trans
...
SELECT account_id, 
    ...
    END AS transaction_type, ROUND(SUM(amount)) AS total_amount  
FROM trans
WHERE account_id = 396
